...
```
Customers.csv          ─┐
Loans.csv               ├─► 01_csv_to_receipts_json.py ─► selected_receipts.json
Repayment_Schedule.csv ─┘               │
                                        ▼
                            02_receipts_json_to_pdf.py ─► output_pdfs/*.pdf
                                        │
                                        ▼
                            03_pdf_to_raw_json.py ─► json_output/*.json  (raw text + word coordinates)
                                        │
                                        ▼
                            04_raw_json_to_structured_json.py ─► output.csv / nested_output.json
                                        │
                                        ▼
                            05_structured_json_to_excel.py ─► output.xlsx (Flattened / Customers / Installments)
```

| Step | Script | Input | Output | Purpose |
|---|---|---|---|---|
| 1 | `src/01_csv_to_receipts_json.py` | `Customers.csv`, `Loans.csv`, `Repayment_Schedule.csv` | `selected_receipts.json` | Joins the three raw sources and builds one flat receipt record per paid/late installment |
| 2 | `src/02_receipts_json_to_pdf.py` | `selected_receipts.json` | `output_pdfs/*.pdf` | Renders each receipt as a "Payment Receipt" PDF (single-invoice or per-customer multi-page mode) |
| 3 | `src/03_pdf_to_raw_json.py` | `*.pdf` | `*.json` | Extracts per-page text, metadata, and word-level coordinates from the generated PDFs |
| 4 | `src/04_raw_json_to_structured_json.py` | PDF-to-JSON output | `output.csv`, nested `output.json` | Reconstructs the two-column invoice layout from word coordinates and regex-parses customer/loan/payment fields; groups installments under each loan |
| 5 | `src/05_structured_json_to_excel.py` | nested JSON | `output.xlsx` | Converts nested records into a 3-sheet Excel workbook (`Flattened`, `Customers`, `Installments`) |
...
- **Data-quality reporting**: `04_raw_json_to_structured_json.py` prints a null-field summary at the end of a run, showing which fields failed to parse and for which files/pages, to make debugging layout issues fast.
...
loan-document-pipeline/
├── src/
│   ├── 01_csv_to_receipts_json.py
│   ├── 02_receipts_json_to_pdf.py
│   ├── 03_pdf_to_raw_json.py
│   ├── 04_raw_json_to_structured_json.py
│   └── 05_structured_json_to_excel.py
├── data/
...
python src/01_csv_to_receipts_json.py

# 2. Render PDF invoices (one file per invoice)
python src/02_receipts_json_to_pdf.py -i selected_receipts.json -o output_pdfs/ --single

#    ...or one PDF per customer, all their invoices as separate pages:
python src/02_receipts_json_to_pdf.py -i selected_receipts.json -o output_pdfs/ --customer

# 3. Extract text + word coordinates from the PDFs (batch mode: pass a folder)
python src/03_pdf_to_raw_json.py output_pdfs/ -o json_output/

# 4. Parse the extracted JSON back into structured records
python src/04_raw_json_to_structured_json.py -i json_output/ -o output.csv --json-output nested_output.json

# 5. Export the nested JSON to a 3-sheet Excel workbook
python src/05_structured_json_to_excel.py -i nested_output.json -o output.xlsx
```
